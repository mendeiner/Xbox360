import { readyConsoles, ensureAllConsoleData } from '../consoles/registry'
...
//
// Built lazily by initMockSocialData() (awaited from AuthContext's mockLogin) instead of at
// module-import time, since registry game data is now itself lazy-loaded — the exported
// MOCK_* stores below are declared empty and filled in place so existing importers keep
// the same object/array reference once initMockSocialData() has run.
...
// Idempotent: awaits registry data so `pickGame` draws from the real catalog, then fills
// every MOCK_* store above in place. Safe to call more than once (subsequent calls reuse
// the same in-flight/completed promise) — called from AuthContext's mockLogin.
export function initMockSocialData() {
  if (!initPromise) {
    initPromise = ensureAllConsoleData().then(() => {
      const pool = allGamesPool()
      const pickGame = seed => pool[seed % pool.length]
