...
const CONSOLE_ID = 'n64'

// ─── Curated rows ──────────────────────────────────────────────────────────────
const FEATURED = [
  'Super Mario 64', 'GoldenEye 007', 'Mario Kart 64', 'Legend of Zelda, The - Ocarina of Time',
  'Legend of Zelda, The - Majora\'s Mask', 'Banjo-Kazooie', 'Banjo-Tooie', 'Perfect Dark',
  'Donkey Kong 64', 'Star Fox 64', 'Paper Mario', 'Kirby 64', 'Diddy Kong Racing',
  'Conker\'s Bad Fur Day', 'F-Zero X',
]
...
const ROWS = [
  ...
  { id:'ano96',      title:'1996 – 1998',      filter: g => g.year >= 1996 && g.year <= 1998 },
  { id:'ano99',      title:'1999 – 2000',      filter: g => g.year >= 1999 && g.year <= 2000 },
  { id:'ano01',      title:'2001+',            filter: g => g.year >= 2001 },
]
...
export default function N64() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f]">
      ...
      <div className="md:ml-[200px]">
        ...
        <div className="sticky top-[57px] z-30 bg-[#0f0f0f]/95 backdrop-blur border-b border-[#1e1e1e] px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-2 flex-shrink-0 text-[#1a1aff] font-black text-[15px] tracking-tight">
              <svg viewBox="0 0 32 32" width="26" height="26" fill="none">
                <rect x="3" y="9" width="26" height="14" rx="6" fill="none" stroke="#1a1aff" strokeWidth="2.5"/>
              </svg>
              N64
            </div>
            ...
            <div className="relative flex-1">
              <input
                ...
                autoComplete="off"
                spellCheck={false}
                className="w-full bg-[#1e1e1e] border border-[#2a2a2a] rounded-full px-4 py-2.5 text-[14px] font-medium outline-none focus:border-[#1a1aff] transition-colors placeholder-gray-600 text-white"
              />
              {search && (
        ...
        <main className="pb-16">
          {(loading || dataLoading) ? (
            ...
            <div className="flex flex-col items-center justify-center h-40 gap-2 text-gray-600">
              ...
              <button onClick={() => { setSearch(''); clearAll() }} className="text-xs text-[#1a1aff] hover:text-white transition-colors">
                Limpar filtros
              </button>
            </div>
          ) : isGrid ? (
                  ...
                  return (
                    <section key={row.id}>
                      <div className="flex items-center gap-3 px-4 mb-2">
                        <h3 className="text-[18px] font-black tracking-tight border-l-[3px] border-[#1a1aff] pl-2.5 leading-none">{row.title}</h3>
      ...
      {/* Stats bar */}
      <div className="fixed bottom-0 left-0 right-0 md:left-[200px] bg-[#111] border-t-2 border-[#1a1aff] px-4 py-2.5 flex items-center gap-3 overflow-x-auto scrollbar-none z-20">
        <Stat color="bg-[#1a1aff]" label={`${stats.joguei} joguei`} />
