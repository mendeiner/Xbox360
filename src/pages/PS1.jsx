...
const CONSOLE_ID = 'ps1'

// ─── Curated rows ──────────────────────────────────────────────────────────────
const FEATURED = [
  'Final Fantasy VII', 'Final Fantasy VIII', 'Metal Gear Solid', 'Resident Evil',
  'Crash Bandicoot', 'Spyro the Dragon', 'Tekken 3', 'Gran Turismo',
  'Castlevania: Symphony of the Night', 'Silent Hill', 'Tomb Raider',
  'Tony Hawk\'s Pro Skater', 'Twisted Metal', 'Ape Escape', 'Chrono Cross', 'Parasite Eve',
]
...
const ROWS = [
  ...
  { id:'ano95',      title:'1995 – 1997',     filter: g => g.year >= 1995 && g.year <= 1997 },
  { id:'ano98',      title:'1998 – 1999',     filter: g => g.year >= 1998 && g.year <= 1999 },
  { id:'ano00',      title:'2000+',           filter: g => g.year >= 2000 },
]
...
export default function PS1() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f]">
      ...
      <div className="md:ml-[200px]">
        ...
        <div className="sticky top-[57px] z-30 bg-[#0f0f0f]/95 backdrop-blur border-b border-[#1e1e1e] px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-2 flex-shrink-0 text-[#8C92AC] font-black text-[15px] tracking-tight">
              <svg viewBox="0 0 32 32" width="26" height="26" fill="none">
                <circle cx="16" cy="16" r="13" fill="none" stroke="#8C92AC" strokeWidth="2.5"/>
              </svg>
              PS1
            </div>
            ...
            <div className="relative flex-1">
              <input
                ...
                autoComplete="off"
                spellCheck={false}
                className="w-full bg-[#1e1e1e] border border-[#2a2a2a] rounded-full px-4 py-2.5 text-[14px] font-medium outline-none focus:border-[#8C92AC] transition-colors placeholder-gray-600 text-white"
              />
              {search && (
        ...
        <main className="pb-16">
          {(loading || dataLoading) ? (
            <div className="flex items-center justify-center h-40">
              <div className="w-6 h-6 border-2 border-[#8C92AC] border-t-transparent rounded-full animate-spin" />
            </div>
            ...
            <div className="flex flex-col items-center justify-center h-40 gap-2 text-gray-600">
              ...
              <button onClick={() => { setSearch(''); clearAll() }} className="text-xs text-[#8C92AC] hover:text-white transition-colors">
                Limpar filtros
              </button>
            </div>
          ) : isGrid ? (
                  ...
                  return (
                    <section key={row.id}>
                      <div className="flex items-center gap-3 px-4 mb-2">
                        <h3 className="text-[18px] font-black tracking-tight border-l-[3px] border-[#8C92AC] pl-2.5 leading-none">{row.title}</h3>
      ...
      {/* Stats bar */}
      <div className="fixed bottom-0 left-0 right-0 md:left-[200px] bg-[#111] border-t-2 border-[#8C92AC] px-4 py-2.5 flex items-center gap-3 overflow-x-auto scrollbar-none z-20">
        <Stat color="bg-[#8C92AC]" label={`${stats.joguei} joguei`} />
