...
const CONSOLE_ID = 'ps3'

// ─── Curated rows ──────────────────────────────────────────────────────────────
const FEATURED = [
  'Uncharted', 'God of War', 'The Last of Us', 'Metal Gear Solid 4', 'Grand Theft Auto IV',
  'Red Dead Redemption', 'Batman: Arkham', 'Gran Turismo 5', "Demon's Souls", 'Dark Souls',
  'Heavy Rain', 'LittleBigPlanet', 'Ratchet & Clank', 'inFamous', 'BioShock',
]
...
export default function PS3() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f]">
      ...
      <div className="md:ml-[200px]">
        ...
        <div className="sticky top-[57px] z-30 bg-[#0f0f0f]/95 backdrop-blur border-b border-[#1e1e1e] px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-2 flex-shrink-0 text-[#00B4D8] font-black text-[15px] tracking-tight">
              <svg viewBox="0 0 32 32" width="26" height="26" fill="none">
                <circle cx="16" cy="16" r="15" fill="#00B4D8"/>
            ...
            <div className="relative flex-1">
              <input
                ...
                autoComplete="off"
                spellCheck={false}
                className="w-full bg-[#1e1e1e] border border-[#2a2a2a] rounded-full px-4 py-2.5 text-[14px] font-medium outline-none focus:border-[#00B4D8] transition-colors placeholder-gray-600 text-white"
              />
              {search && (
        ...
        <main className="pb-16">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-40 gap-2 text-gray-600">
              ...
              <button onClick={() => { setSearch(''); clearAll() }} className="text-xs text-[#00B4D8] hover:text-white transition-colors">
                Limpar filtros
              </button>
            </div>
          ) : isGrid ? (
                  ...
                  return (
                    <section key={row.id}>
                      <div className="flex items-center gap-3 px-4 mb-2">
                        <h3 className="text-[18px] font-black tracking-tight border-l-[3px] border-[#00B4D8] pl-2.5 leading-none">{row.title}</h3>
      ...
      {/* Stats bar */}
      <div className="fixed bottom-0 left-0 right-0 md:left-[200px] bg-[#111] border-t-2 border-[#00B4D8] px-4 py-2.5 flex items-center gap-3 overflow-x-auto scrollbar-none z-20">
        <Stat color="bg-[#00B4D8]" label={`${stats.joguei} joguei`} />
