...
const CONSOLE_ID = 'wii'

// ─── Curated rows ──────────────────────────────────────────────────────────────
const FEATURED = [
  'Super Mario Galaxy', 'Mario Kart Wii', 'Super Smash Bros. Brawl', 'Wii Sports', 'Metroid Prime 3',
  'The Legend of Zelda: Twilight Princess', 'Super Paper Mario', 'Donkey Kong Country Returns',
  'Kirby\'s Return to Dream Land', 'Xenoblade Chronicles', 'No More Heroes', 'Punch-Out!!',
  'Wii Fit', 'Just Dance', 'Mario Party 8',
]

const ROWS = [
  { id:'destaque',   title:'Em Destaque',     filter: g => FEATURED.some(t => g.title.includes(t)) },
  { id:'dl',         title:'Com Download',     filter: g => !!g.dl },
  { id:'wiiware',    title:'WiiWare',          filter: g => g.type === 'wiiware' },
  { id:'vc',         title:'Virtual Console',  filter: g => g.type === 'virtual_console' },
  ...
  { id:'tiro',       title:'Tiro & FPS',       filter: g => (g.genre||[]).includes('Tiro') || (g.genre||[]).includes('FPS') },
  { id:'luta',       title:'Luta',             filter: g => (g.genre||[]).includes('Luta') || (g.genre||[]).includes("Beat 'em up") },
  ...
  { id:'musica',     title:'Música',           filter: g => (g.genre||[]).includes('Música') },
  ...
  { id:'motionplus', title:'Motion Plus',      filter: g => g.motionPlus === true },
  { id:'ano06',      title:'2006 – 2008',      filter: g => g.year >= 2006 && g.year <= 2008 },
  { id:'ano09',      title:'2009 – 2011',      filter: g => g.year >= 2009 && g.year <= 2011 },
  { id:'ano12',      title:'2012+',            filter: g => g.year >= 2012 },
]
...
export default function Wii() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f]">
      ...
      <div className="md:ml-[200px]">
        ...
        <div className="sticky top-[57px] z-30 bg-[#0f0f0f]/95 backdrop-blur border-b border-[#1e1e1e] px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-2 flex-shrink-0 text-[#1492E6] font-black text-[15px] tracking-tight">
              <svg viewBox="0 0 32 32" width="26" height="26" fill="none">
                <ellipse cx="16" cy="16" rx="14" ry="10" fill="none" stroke="#1492E6" strokeWidth="2.5"/>
              </svg>
              Wii
            </div>
            ...
            <div className="relative flex-1">
              <input
                ...
                autoComplete="off"
                spellCheck={false}
                className="w-full bg-[#1e1e1e] border border-[#2a2a2a] rounded-full px-4 py-2.5 text-[14px] font-medium outline-none focus:border-[#1492E6] transition-colors placeholder-gray-600 text-white"
              />
              {search && (
        ...
        <main className="pb-16">
          {(loading || dataLoading) ? (
            <div className="flex items-center justify-center h-40">
              <div className="w-6 h-6 border-2 border-[#1492E6] border-t-transparent rounded-full animate-spin" />
            </div>
            ...
            <div className="flex flex-col items-center justify-center h-40 gap-2 text-gray-600">
              ...
              <button onClick={() => { setSearch(''); clearAll() }} className="text-xs text-[#1492E6] hover:text-white transition-colors">
                Limpar filtros
              </button>
            </div>
          ) : isGrid ? (
                  ...
                  return (
                    <section key={row.id}>
                      <div className="flex items-center gap-3 px-4 mb-2">
                        <h3 className="text-[18px] font-black tracking-tight border-l-[3px] border-[#1492E6] pl-2.5 leading-none">{row.title}</h3>
      ...
      {/* Stats bar */}
      <div className="fixed bottom-0 left-0 right-0 md:left-[200px] bg-[#111] border-t-2 border-[#1492E6] px-4 py-2.5 flex items-center gap-3 overflow-x-auto scrollbar-none z-20">
        <Stat color="bg-[#1492E6]" label={`${stats.joguei} joguei`} />
