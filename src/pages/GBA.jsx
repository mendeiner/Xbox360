...
const CONSOLE_ID = 'gba'

// ─── Curated rows ──────────────────────────────────────────────────────────────
const FEATURED = [
  'Pokemon - Ruby Version', 'Pokemon - FireRed Version', 'Pokemon - Emerald Version',
  'Metroid Fusion', 'Metroid - Zero Mission', 'Mario Kart - Super Circuit', 'Golden Sun',
  'Advance Wars', 'Fire Emblem', 'Mario & Luigi - Superstar Saga', 'Castlevania - Aria of Sorrow',
  'Kirby - Nightmare in Dream Land', 'Final Fantasy Tactics Advance', 'Legend of Zelda, The - The Minish Cap',
  'Wario Land 4', 'Sonic Advance',
]
...
const ROWS = [
  ...
  { id:'linkcable', title:'Link Cable',      filter: g => g.localMP },
  { id:'ano01',     title:'2001 – 2003',     filter: g => g.year >= 2001 && g.year <= 2003 },
  { id:'ano04',     title:'2004 – 2005',     filter: g => g.year >= 2004 && g.year <= 2005 },
...
function MiniCard({ game, console: console_, status, onClick }) {
  ...
  const border =
    ...
    status.zerado       ? 'border-red-600'    :
    status.joguei       ? 'border-[#7B2D8E]' :
    status.quero        ? 'border-purple-600' : 'border-transparent'

  return (
    <div className="flex-shrink-0 cursor-pointer group" onClick={() => onClick(game)}>
      <div className={`relative w-[100px] h-[74px] rounded-md bg-[#1a1a1a] border-2 overflow-hidden ${border}
      ...
      <p className="mt-1 text-[9px] text-gray-500 text-center w-[100px] truncate">{game.title}</p>
    </div>
  )
}
...
export default function GBA() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f]">
      ...
      <div className="md:ml-[200px]">
        ...
        <div className="sticky top-[57px] z-30 bg-[#0f0f0f]/95 backdrop-blur border-b border-[#1e1e1e] px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-2 flex-shrink-0 text-[#7B2D8E] font-black text-[15px] tracking-tight">
              <svg viewBox="0 0 32 32" width="26" height="26" fill="none">
                <rect x="2" y="6" width="28" height="20" rx="4" fill="#7B2D8E"/>
                <rect x="6" y="10" width="14" height="12" rx="1.5" fill="#1a1a1a"/>
                <circle cx="25" cy="12" r="2" fill="white" opacity="0.8"/>
                <circle cx="25" cy="18" r="2" fill="white" opacity="0.5"/>
              </svg>
              GBA
            </div>
            ...
            <div className="relative flex-1">
              <input
                ...
                autoComplete="off"
                spellCheck={false}
                className="w-full bg-[#1e1e1e] border border-[#2a2a2a] rounded-full px-4 py-2.5 text-[14px] font-medium outline-none focus:border-[#7B2D8E] transition-colors placeholder-gray-600 text-white"
              />
              {search && (
        ...
        <main className="pb-16">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-40 gap-2 text-gray-600">
              ...
              <button onClick={() => { setSearch(''); clearAll() }} className="text-xs text-[#7B2D8E] hover:text-white transition-colors">
                Limpar filtros
              </button>
            </div>
          ) : isGrid ? (
                ...
                return (
                  <section key={row.id}>
                    <div className="flex items-center gap-3 px-4 mb-2">
                      <h3 className="text-[18px] font-black tracking-tight border-l-[3px] border-[#7B2D8E] pl-2.5 leading-none">{row.title}</h3>
      ...
      {/* Stats bar */}
      <div className="fixed bottom-0 left-0 right-0 md:left-[200px] bg-[#111] border-t-2 border-[#7B2D8E] px-4 py-2.5 flex items-center gap-3 overflow-x-auto scrollbar-none z-20">
        <Stat color="bg-[#7B2D8E]" label={`${stats.joguei} joguei`} />
        <Stat color="bg-red-700"   label={`${stats.zerado} zerado`} />
