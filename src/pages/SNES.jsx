...
const CONSOLE_ID = 'snes'

// ─── Curated rows ──────────────────────────────────────────────────────────────
const FEATURED = [
  'Super Mario World', 'The Legend of Zelda', 'Chrono Trigger', 'Super Metroid', 'Donkey Kong Country',
  'Final Fantasy III', 'Super Mario Kart', 'Street Fighter II', 'Mega Man X', 'Kirby Super Star',
  'Star Fox', 'Earthbound', 'Super Castlevania IV', 'Contra III',
]
...
const ROWS = [
  ...
  { id:'ano90',     title:'1990 – 1992',     filter: g => g.year >= 1990 && g.year <= 1992 },
  { id:'ano93',     title:'1993 – 1995',     filter: g => g.year >= 1993 && g.year <= 1995 },
  { id:'ano96',     title:'1996+',           filter: g => g.year >= 1996 },
]
...
export default function SNES() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f]">
      ...
      <div className="md:ml-[200px]">
        ...
        <div className="sticky top-[57px] z-30 bg-[#0f0f0f]/95 backdrop-blur border-b border-[#1e1e1e] px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-2 flex-shrink-0 text-[#E60012] font-black text-[15px] tracking-tight">
              <svg viewBox="0 0 32 32" width="26" height="26" fill="none">
                <circle cx="16" cy="16" r="15" fill="#E60012"/>
                <text x="16" y="21" textAnchor="middle" fontSize="13" fontWeight="900" fill="white">S</text>
              </svg>
              SNES
            </div>
            ...
            <div className="relative flex-1">
              <input
                ...
                autoComplete="off"
                spellCheck={false}
                className="w-full bg-[#1e1e1e] border border-[#2a2a2a] rounded-full px-4 py-2.5 text-[14px] font-medium outline-none focus:border-[#E60012] transition-colors placeholder-gray-600 text-white"
              />
              {search && (
        ...
        <main className="pb-16">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-40 gap-2 text-gray-600">
              ...
              <button onClick={() => { setSearch(''); clearAll() }} className="text-xs text-[#E60012] hover:text-white transition-colors">
                Limpar filtros
              </button>
            </div>
          ) : isGrid ? (
                ...
                return (
                  <section key={row.id}>
                    <div className="flex items-center gap-3 px-4 mb-2">
                      <h3 className="text-[18px] font-black tracking-tight border-l-[3px] border-[#E60012] pl-2.5 leading-none">{row.title}</h3>
      ...
      {/* Stats bar */}
      <div className="fixed bottom-0 left-0 right-0 md:left-[200px] bg-[#111] border-t-2 border-[#E60012] px-4 py-2.5 flex items-center gap-3 overflow-x-auto scrollbar-none z-20">
        <Stat color="bg-[#E60012]" label={`${stats.joguei} joguei`} />
