"""
Fetch and compress Nintendo 64 cover images.
  - Source: github.com/libretro-thumbnails/Nintendo_-_Nintendo_64, Named_Boxarts/<filename>.png.
    Same source family already used for GBA/Wii.
  ...
  - A number of source PNGs in this repo are themselves redirect-pointer text files (case-
    insensitive filesystem collisions in the source repo, same quirk documented for Wii in
    CLAUDE.md) containing the real filename as plain text instead of image bytes. When a
    download isn't a valid image, this script checks for that case and follows the pointer.
  - Output: public/covers/n64/<gameId>.webp
  - Resize: max height 400px, keep aspect ratio
  - Format: WebP quality 80

Run from the game-tracker project root:
  python3 compress_covers_n64.py
"""
...
COVERS_FILE = Path("src/data/n64/covers_map.js")
LIBRETRO_BASE = "https://raw.githubusercontent.com/libretro-thumbnails/Nintendo_-_Nintendo_64/master/Named_Boxarts/{name}.png"
DEST = Path("public/covers/n64")
MAX_H = 400
QUALITY = 80
