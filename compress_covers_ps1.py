"""
Fetch and compress PlayStation 1 cover images.
  - Source: github.com/libretro-thumbnails/Sony_-_PlayStation, Named_Boxarts/<filename>.png.
    Same source family already used for GBA/Wii/N64/GameCube.
  ...
  - A number of source PNGs in this repo family are themselves redirect-pointer text files
    ...
    Wii/N64/GameCube in CLAUDE.md) containing the real filename as plain text instead of
    image bytes. When a download isn't a valid image, this script checks for that case and
    follows the pointer.
  - Every image in this repo is a fixed 512x512 square canvas (front-cover-only scan, not
    portrait jewel-case art) — verified by sampling actual downloaded pixel dimensions before
    writing this script. Registry entry must set coverAspect: 'square' (same convention as
    GBA/3DS), or the square art gets force-cropped by the default portrait slot.
  - Output: public/covers/ps1/<gameId>.webp
  - Resize: max height 400px, keep aspect ratio
  - Format: WebP quality 80

Run from the game-tracker project root:
  python3 compress_covers_ps1.py
"""
...
COVERS_FILE = Path("src/data/ps1/covers_map.js")
LIBRETRO_BASE = "https://raw.githubusercontent.com/libretro-thumbnails/Sony_-_PlayStation/master/Named_Boxarts/{name}.png"
DEST = Path("public/covers/ps1")
MAX_H = 400
QUALITY = 80
...
for game_id, raw_name in entries:
    ...
    if len(data) < 300 and not data.startswith(b"\x89PNG"):
        try:
