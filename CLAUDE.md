...
A personal multi-console game-tracking web app (React + Vite + Tailwind + Supabase), in Portuguese (pt-BR UI). Users mark games as `joguei` (played) / `zerado` (beaten) / `cem_porcento` (100%) / `quero` (want to play), rate them, and find archive.org download links for retail/XBLA/Kinect titles. Xbox 360, PS2, PS3, SNES, NSW (Switch), GBA, Wii and PS4 are implemented; remaining consoles (N64, GameCube, Xbox One/Series, PC, GBC, 3DS) are listed on the home screen as "Em breve" (coming soon) placeholders. The "Meu Catálogo" grid and "Meus Jogos" section on `Home.jsx` (not a separate `/dashboard` route/page — there isn't one) show a logged-in user's marked games from every `ready` console grouped by console.
...
`/` → `Login` (or redirect to `/home` if authed) · `/home` → `Home` (console picker + cross-console collection, behind `PrivateRoute`) · `/xbox360` → `Xbox360` · `/ps2` → `PS2` · `/ps3` → `PS3` · `/snes` → `SNES` · `/nsw` → `NSW` · `/gba` → `GBA` · `/wii` → `Wii`. Note the console pages are **not** wrapped in `PrivateRoute`, so each page itself has to handle the no-`user` case (it does, by skipping the status fetch and disabling write actions).
